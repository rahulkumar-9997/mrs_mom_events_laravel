                            ...
                            <div class="sort-div role=" tabpanel">
                                <ul class="list-unstyled list-group sortable stage ui-sortable" id="sortable_foundation_image">
                                    ...
                                    <li class="d-flex align-items-center justify-content-between list-group-item ui-sortable-handle"
                                        ...
                                        <span class="float-end d-flex column-gap-1">
                                            <button type="button" class="btn btn-sm btn-primary rotate-image" data-id="{{ $image->id }}" data-angle="90">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(".rotate-image").on("click", function() {
            ...
            $.ajax({
                url: "{{ route('gallery-image.rotate', '') }}/" + imageId,
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    angle: angle
                ...
                success: function(response) {
                    if (response.success) {
                        $(`img[data-id="${imageId}"]`).attr("src", response.image_url + "?t=" + new Date().getTime());
                        showSuccess(response.message);
                    } else {
                        showSuccess(response.message);
                    }
                ...
                error: function(xhr) {
                    showSuccess('An error occurred while rotating the image.');
                }
            });
        });
        /**gallery image rotation */
